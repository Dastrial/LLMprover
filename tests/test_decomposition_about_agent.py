"""Tests for llmprover.prover_agents.decomposition_about_agent."""
...
SEARCH_SYSTEM = cached_system_prompt(DECOMPOSITION_ABOUT_SPEC)
...
def test_prove_runs_search_then_about_then_decomposition() -> None:
    ...
    agent = DecompositionAboutAgent(mock_model)
    ...
    if not after.endswith("\n"):
        after = f"{after}\n"
    assert search_call[0] == PromptMessage.text(
        "system", SEARCH_SYSTEM, cache_breakpoint=True
    )
    ...
    assert proof_call[0] == PromptMessage.text(
        "system", SEARCH_SYSTEM, cache_breakpoint=True
    )
